...
def configuration_keygen():
    ...
    try:
        ...
        cmd = ['wg genkey | sudo tee {} | wg pubkey | sudo tee {}'.format(
            key_location[0], key_location[1])]
        result, err = charms.sshproxy._run(cmd)
    except:
        log('command failed:' + err)
...
def getVnfIp():
    ...
    try:
        ...
        log("using interface: " +
            str(interfacesAndIps[0])+" with MAC "+interfacesAndMACs[interfacesAndIps[0][0]])
        # try:
        #     cmd = ['sudo tc qdisc add dev {} root cake bandwidth {}mbit'.format(key, bandwidth)]
        #     result, err = charms.sshproxy._run(cmd)
        #     log("enabled tc-cake to interface")
        ...
        result = json.dumps({"vsiId": vsiId, "publicKey": publicKey, "vnfIp": vnfMgmtIp,
                             "tunnelId": tunnelId, "vnfMAC": interfacesAndMACs[interfacesAndIps[0][0]], "gwMAC": gwMAC})
