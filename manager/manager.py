                    ...
                    for vsiId, composition in self.vs.items():
                        for component, componentData in composition.items():
                            if componentData["sliceEnabled"]:
                                message = {"vsiId": vsiId, "msgType": "getNsiInfo", "data": {
                                    "domainId": componentData["domainId"], "nsiId": component}}
                            else:
                                message = {"vsiId": vsiId, "msgType": "getNsInfo", "data": {
                                    "domainId": componentData["domainId"], "nsId": component}}
                            logging.info("Polling action: {}".format(message))
                            self.messaging.publish2Queue(
                                "vsDomain", json.dumps(message))
        pass
...
class CSMF():
    def __init__(self, vsiId, vsiRequest, pollingThread):
        ...
        self.has_been_created = False    # if the service has been createds
        ...
        statusUpdate = {"msgType": "statusUpdate", "data": {"vsiId": self.vsiId, "status": "creating",
                                                            "message": "Created Management Function, waiting to receive all necessary information"}}
    ...
    def processAction(self, data):
        ...
        if data["msgType"] == "nsInfo":
            statusUpdate = {"msgType": "statusUpdate", "data": {
                "vsiId": self.vsiId, "status": data["data"]["nsInfo"]["operational-status"], "message": data["data"]["nsId"]+": "+data["data"]["nsInfo"]["detailed-status"]}}
            self.messaging.publish2Queue(
                "vsCoordinator", json.dumps(statusUpdate))
            if "running" in data["data"]["nsInfo"]["operational-status"].lower():
                if serviceComposition == {}:
                    redis.setKeyValue("serviceComposition", self.vsiId, json.dumps(
                        {data["data"]["nsId"]: {"status": "instantiated"}}))
                else:
                    serviceComposition[data["data"]["nsId"]
                                       ]["status"] = "instantiated"
                    redis.setKeyValue("serviceComposition",
                                      self.vsiId, json.dumps(serviceComposition))

                tunnelActive = redis.getHashValue(
                    "interdomainTunnel", self.vsiId).decode("UTF-8")
                if self.interdomain and tunnelActive == "off":
                    ...
                    # me
                    #print("primitiveData", primitiveData)
            ...
            print("nsiInfo ->", data)
            statusUpdate = {"msgType": "statusUpdate", "data": {"vsiId": self.vsiId, "status": data["data"]["nsiInfo"][
                "operational-status"], "message": data["data"]["nsiId"]+": "+data["data"]["nsiInfo"]["detailed-status"]}}
            self.messaging.publish2Queue(
                "vsCoordinator", json.dumps(statusUpdate))
            if "running" in data["data"]["nsiInfo"]["operational-status"].lower():
                if serviceComposition == {}:
                    redis.setKeyValue("serviceComposition", self.vsiId, json.dumps(
                        {data["data"]["nsiId"]: {"status": "instantiated"}}))
                else:
                    serviceComposition[data["data"]["nsiId"]
                                       ]["status"] = "instantiated"
                    redis.setKeyValue("serviceComposition",
                                      self.vsiId, json.dumps(serviceComposition))
                ...
                tunnelActive = redis.getHashValue(
                    "interdomainTunnel", self.vsiId).decode("UTF-8")
                if self.interdomain and tunnelActive == "off":
                    primitiveData = {"data": {"primitiveName": "getvnfinfo",
                                              "primitiveTarget": data["data"]["nsiId"], "primitiveInternalTarget": "1"}}
                    #print("primitiveData", primitiveData)
                    # me
                    #self.processVsiPrimitive(primitiveData)
            elif "terminated" in data["data"]["nsiInfo"]["operational-status"].lower():
            ...
            if serviceComposition == {}:
                redis.setKeyValue("serviceComposition", self.vsiId, json.dumps(
                    {nfvoData["componentName"]: {"nfvoId": nfvoData["componentId"]}}))
            else:
                serviceComposition[nfvoData["componentName"]
                                   ]["nfvoId"] = nfvoData["componentId"]
                redis.setKeyValue("serviceComposition",
                                  self.vsiId, json.dumps(serviceComposition))
            ...
            if data["data"]["primitiveName"] == "getvnfinfo":
                try:
                    ...
                    logging.error(
                        "Error after fetching Tunnel VNF info: "+str(e))
            return
            ...
            createVsi = redis.getHashValue(
                "createVSI", self.vsiId).decode("UTF-8")

            if createVsi == "create" and set(["catalogueInfo", "domainInfo", "tenantInfo", "placementInfo"]).issubset(receivedData) and not self.has_been_created:
                print("instantiating VSI ->", data)
    ...
    def instantiateVSI(self):
        ...
        if "domainPlacements" in vsiRequest["data"]:
            for placement in vsiRequest["data"]["domainPlacements"]:
                if placement["domainId"] in domainInfo["data"]:
                    domainPlacements[placement["componentName"]
                                     ] = placement["domainId"]
        ...
        for creationData in placementData:
            ...
            componentName = str(
                self.vsiId)+"_"+str(composingComponentId)+"-"+vsiRequest["data"]["name"]
            ...
            if creationData["sliceEnabled"] and creationData["nstId"] != None:
                ...
                message = {"msgType": "instantiateNsi", "data": {"name": componentName,
                                                                 "description": vsiRequest["data"]["description"], "domainId": domainId, "nstId": creationData["nstId"]}}
                if "additionalConf" in vsiRequest["data"] and vsiRequest["data"]["additionalConf"] != "":
                    ...
                    if catalogueInfo["data"]["vs_blueprint_info"]["vs_blueprint"]["inter_site"]:
                        ...
                        #                            member['additionalParams'].update({'vsi_id': str(
                        #                                self.vsiId), 'tunnel_address': '10.100.100.'+str(composingComponentId)+'/24', 'tunnel_id': componentName})
                        #                        else:
                        #                            member['additionalParams'] = {'vsi_id': str(
                        #                                self.vsiId), 'tunnel_address': '10.100.100.'+str(composingComponentId)+'/24', 'tunnel_id': componentName}
                        #                        internalConfigured = True
                        #                        configured = True
                        #                        break
                        #                if not internalConfigured:
                        #                    subnet['additionalParamsForVnf'].append({'member-vnf-index': '1', 'additionalParams': {'vsi_id': str(
                        #                        self.vsiId), 'tunnel_address': '10.100.100.'+str(composingComponentId)+'/24', 'tunnel_id': componentName}})
                        #                    configured = True
                        #            else:
                        #                subnet['additionalParamsForVnf'] = [{'member-vnf-index': '1', 'additionalParams': {'vsi_id': str(
                        #                    self.vsiId), 'tunnel_address': '10.100.100.'+str(composingComponentId)+'/24', 'tunnel_id': componentName}}]
                        #                configured = True
                        #                break
                        #    if not configured:
                        #        config["netslice-subnet"].append({'id': 'interdomain-tunnel-peer', 'additionalParamsForVnf': [{'member-vnf-index': '1', 'additionalParams': {
                        #                                         'vsi_id': str(self.vsiId), 'tunnel_address': '10.100.100.'+str(composingComponentId)+'/24', 'tunnel_id': componentName}}]})
                        #else:
                        #    config["netslice-subnet"] = [{'id': 'interdomain-tunnel-peer', 'additionalParamsForVnf': [{'member-vnf-index': '1', 'additionalParams': {
                        #        'vsi_id': str(self.vsiId), 'tunnel_address': '10.100.100.'+str(composingComponentId)+'/24', 'tunnel_id': componentName}}]}]
            ...
            #    if componentName in domainPlacements:
            #        domainId = domainPlacements[componentName]
            #    message = {"msgType": "instantiateNs", "data": {
            #        "name": componentName, "domainId": domainId, "nsdId": creationData["nsdId"]}}
            #    if "additionalConf" in vsiRequest["data"] and vsiRequest["data"]["additionalConf"] != "":
            ...
            #                config["additionalParamsForVnf"].append({'member-vnf-index': '1', 'additionalParams': {'vsi_id': str(
            #                    self.vsiId), 'tunnel_address': '10.100.100.'+str(composingComponentId)+'/24', 'tunnel_id': componentName}})
            #            else:
            #                config["additionalParamsForVnf"] = [{'member-vnf-index': '1', 'additionalParams': {'vsi_id': str(
            #                    self.vsiId), 'tunnel_address': '10.100.100.'+str(composingComponentId)+'/24', 'tunnel_id': componentName}}]
            #            confStr = yaml.safe_dump(config)
            #        message["data"]["additionalConf"] = confStr
            #    else:
            ...
            serviceComposition = redis.getHashValue(
                "serviceComposition", self.vsiId)
            if serviceComposition == None:
                serviceComposition = {componentName: {
                    "sliceEnabled": creationData["sliceEnabled"], "domainId": domainId, "status": "instantiating"}}
                redis.setKeyValue("serviceComposition",
                                  self.vsiId, json.dumps(serviceComposition))
            else:
        ...
        statusUpdate = {"msgType": "statusUpdate", "data": {"vsiId": self.vsiId, "status": "deploying",
                                                            "message": "Sent all instantiation requests to the appropriate domains"}}
        messaging.publish2Queue("vsCoordinator", json.dumps(statusUpdate))
    ...
    def processVsiPrimitive(self, data):
        print("processVsiPrimitive ->", data)
        ...
        if createVsi == "alreadyCreated":
            ...
            if data["data"]["primitiveName"] not in actions:
                statusUpdate = {"msgType": "statusUpdate", "data": {"vsiId": self.vsiId, "status": "Invalid Primitive",
                    "message": "Primitive "+data["data"]["primitiveName"]+" is not defined in the VSB."}}
                self.messaging.publish2Queue("vsCoordinator", json.dumps(statusUpdate))
                return
            ...
            if data["data"]["primitiveTarget"] in serviceComposition:
                if serviceComposition[data["data"]["primitiveTarget"]]["sliceEnabled"]:
                    ...
                    message = {"msgType": "actionNs", "vsiId": self.vsiId, "data": {
                        "domainId": serviceComposition[data["data"]["primitiveTarget"]]["domainId"], "nsId": serviceComposition[data["data"]["primitiveTarget"]]["nfvoId"], "additionalConf": conf}}
                else:
                    conf = {"member_vnf_index": data["data"]["primitiveInternalTarget"],
                            "primitive": data["data"]["primitiveName"], "primitive_params": {}}
                    ...
                    message = {"msgType": "actionNs", "vsiId": self.vsiId, "data": {
                        "domainId": serviceComposition[data["data"]["primitiveTarget"]]["domainId"], "nsId": data["data"]["primitiveTarget"], "additionalConf": conf}}
            message["data"]["primitiveName"] = data["data"]["primitiveName"]
            self.messaging.publish2Queue("vsDomain", json.dumps(message))
        else:
            statusUpdate = {"msgType": "statusUpdate", "data": {
                "vsiId": self.vsiId, "status": "Invalid Primitive Trigger", "message": "Triggered primitive before VSI deployment finalized."}}
            self.messaging.publish2Queue("vsCoordinator", json.dumps(statusUpdate))
        print("primitive message ->", message)
    ...
    def interdomainHandler(self, data):
        ...
        if interdomainInfo == None:
            ...
            redis.setKeyValue("interdomainInfo", self.vsiId,
                              json.dumps(interdomainInfo))
        else:
        ...
        if len(interdomainInfo) == len(serviceComposition):
            ...
            if tunnelActive == "off":
                ...
                if "addpeer" not in actions:
                    statusUpdate = {"msgType": "statusUpdate", "data": {
                        "vsiId": self.vsiId, "status": "Invalid Primitive", "message": "addpeer primitive not present in the blueprint."}}
                    self.messaging.publish2Queue(
                        "vsCoordinator", json.dumps(statusUpdate))
                    return
                    ...
                    for componentReceive, infoReceive in interdomainInfo.items():
                        if componentSend != componentReceive:
                            if serviceComposition[componentReceive]["sliceEnabled"]:
                                ...
                                conf = {"member_vnf_index": "1", "primitive": "addpeer", "primitive_params": {
                                    'peer_endpoint': infoSend["vnfIp"], 'peer_key': infoSend["publicKey"]}}
                                for param in actions["addpeer"]:
                                    if "parameter_default_value" in param:
                                        if param["parameter_default_value"] != None and param["parameter_default_value"] != "":
                                            conf["primitive_params"][param["parameter_id"]
                                                                     ] = param["parameter_default_value"]

                                message = {"msgType": "actionNs", "data": {"primitiveName": "addpeer", "domainId": serviceComposition[
                                    componentReceive]["domainId"], "nsId": serviceComposition[componentReceive]["nfvoId"], "additionalConf": conf}}
                            else:
                                conf = {"member_vnf_index": "1", "primitive": "addpeer", "primitive_params": {
                                    'peer_endpoint': infoSend["vnfIp"], 'peer_key': infoSend["publicKey"]}}
                                for param in actions["addpeer"]:
                            ...
                            logging.info(
                                "Sending interdomain addpeer action: {}".format(message))
                            self.messaging.publish2Queue(
                                "vsDomain", json.dumps(message))

                #requests.post("http://10.0.12.117:9999/stopTimer/1", data={"timestamp":str(round(time.time()*1000))})

    def wgMeshHandler(self, data):
        print("wgMeshHandler")

        for peer1 in data["peers"]:
            for peer2 in data["peers"]:

                # don't add peer to itself
                if peer1["component_id"] == peer2["component_id"]:
                    continue
                ...
                message = {
                    ...
                    'vsiId': f"{data['vsiId']}",
                    'data': {
                        'primitiveName': 'add-peer',
                        'primitiveTarget': peer1["component_id"],
                        'primitiveInternalTarget': '1',
                        'primitiveParams': {
                            'peer_endpoint': peer2["peer_endpoint"],
                            'peer_public_key': peer2["peer_public_key"],
                            'allowed_networks': peer2["allowed_networks"]
                        }
                    }
                }
                self.processVsiPrimitive(message)
    ...
    def deleteVsi(self, force=False):
        ...
        for component, componentData in serviceComposition.items():
            ...
            if componentData["sliceEnabled"]:
                message = {"vsiId": self.vsiId, "msgType": "deleteNsi", "data": {
                    "domainId": componentData["domainId"], "nsiId": component}}
            else:
                message = {"vsiId": self.vsiId, "msgType": "deleteNs", "data": {
                    "domainId": componentData["domainId"], "nsId": component}}
            self.messaging.publish2Queue("vsDomain", json.dumps(message))

        redis.setKeyValue("serviceComposition", self.vsiId,
                          json.dumps(serviceComposition))

        statusUpdate = {"msgType": "statusUpdate", "data": {"vsiId": self.vsiId,
                                                            "status": "terminating", "message": "Terminating Vertical Service Instance."}}
        self.messaging.publish2Queue("vsCoordinator", json.dumps(statusUpdate))
    ...
    def tearDown(self, componentName, force=False):
        ...
        if terminated or force:
            ...
            statusUpdate = {"msgType": "statusUpdate", "data": {
                "vsiId": self.vsiId, "status": "terminated", "message": "Vertical Service Instance Terminated."}}
            self.messaging.publish2Queue(
                "vsCoordinator", json.dumps(statusUpdate))
...
def forceDelete(vsiId):
    ...
    messaging = Messaging()
    statusUpdate = {"msgType": "statusUpdate", "data": {
        "vsiId": vsiId, "status": "terminated", "message": "Vertical Service Instance Terminated."}}
    messaging.publish2Queue("vsCoordinator", json.dumps(statusUpdate))
